...
| PWR 길게 (0.7초+) | 일시정지 / 재개 |
| PWR 아주 길게 (2초+) | 전원 끔. 다시 PWR 을 누르면 켜진다 |
...
- SHTC3 온도는 Waveshare 예제와 같이 **4도를 뺀다**. 센서가 ESP32 바로 옆이라
  자체 발열만큼 높게 읽힌다. `SHTC3_TEMP_OFFSET_C` 로 조정한다.
...
EPD 전원 레일도 끄지 않는다. 전원을 끊으면 패널의 이전 이미지가 사라져 매번
전체 갱신(1.4초 + 번쩍임)을 해야 한다. 하이버네이트된 패널은 1µA 수준이라 켜
두는 편이 싸다. 깨어난 뒤에는 `uiBegin(false)` 로 부른다 — `initial=true` 면
GxEPD2 가 다음 갱신을 전체 갱신으로 승격시켜 부분 갱신을 쓰려던 의도가 무너진다.
잔상은 30분마다 한 번 전체 갱신으로 턴다.
