...
**PWR 로 끄면 다음 부팅에서 Wi-Fi 를 못 잡을 수 있다.** 실제로 겪은 증상이다 —
인사도 없이 사라지면 공유기가 한동안 예전 세션을 붙들고 있고, 같은 MAC 으로 다시
붙으러 가면 4-way 핸드셰이크가 막힌다(reason 15). 배터리가 바닥나서 꺼지는 경우도
마찬가지다. 붙지 못한 채로 부팅하면 30초마다 계속 다시 시도하므로 결국은 알아서
살아나지만, 그 전에 설정 포털이 한 번 뜬다.
