...
깨어난 뒤 갱신은 **전체 갱신**이다(`uiBegin()`, `initial=true`). 1분마다
1.4초씩 번쩍이는 게 아깝지만, 부분 갱신은 동작하지 않는다.
`GxEPD2_EPD::init()` 은 `initial` 값과 무관하게 항상 `_reset()` 을 호출하고,
리셋 뒤 컨트롤러가 이전 이미지를 갖고 있다는 보장이 없다. `initial=false` 로
부분 갱신을 요청하면 화면이 아예 바뀌지 않는다. 실제로 그렇게 바꿨다가
시계가 멈춘 적이 있다.
