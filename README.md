...
- SHTC3 온도는 자체 발열만큼 높게 읽힌다. 센서가 ESP32 바로 옆이기 때문이다.
  다만 그 양은 **고정이 아니라 얼마나 오래 깨어 있었느냐에 달려 있다.**

| 상태 | 보정 | 이유 |
|---|---|---|
| 재생 중 (`SHTC3_TEMP_OFFSET_RUN_C`) | −4.0℃ | Wi-Fi·CPU 가 계속 돌아 보드가 덥다. Waveshare 예제가 쓰는 값 |
| 시계 모드 (`SHTC3_TEMP_OFFSET_IDLE_C`) | 0℃ | 1분 중 1초만 깨어 있어 자체 발열이 없다. 여기에 4도를 빼면 오히려 실제보다 낮게 나온다 |

  둘 다 실측 없이 정한 어림값이다. 정확히 맞추려면 시리얼 로그의 `raw` 값을
  실제 온도계와 비교해서 상수를 고치면 된다.
