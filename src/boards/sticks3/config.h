...
//
// ePaper 판은 10% 에서 끈다. 그쪽은 꺼진 뒤에도 시계 모드로 1분마다 깨어나
// 계속 조금씩 먹으므로, 여유를 남겨 두지 않으면 며칠 뒤 셀이 과방전된다.
//
// 이 보드는 시계 기능이 없다. M5PM1 이 진짜 전원 차단을 지원해서 꺼지면 거의
// 아무것도 먹지 않으므로, 남겨 둘 이유가 없다. 끝까지 쓴다.
//
// 다만 곡선상 1% 는 3.33V 근처다. 이 부근에서는 Wi-Fi 송신 피크에 전압이
// 주저앉아 우아하게 꺼지기 전에 그냥 죽을 수 있고, 리튬 셀을 매번 여기까지
// 쓰면 수명도 준다. 그걸 알고 택한 값이다.
constexpr uint8_t BATT_CUTOFF_PERCENT = 1;
